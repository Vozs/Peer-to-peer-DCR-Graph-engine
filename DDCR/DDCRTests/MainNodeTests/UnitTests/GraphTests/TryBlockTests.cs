...
namespace DDCRTests.UnitTests
{
    class TryBlockTests
    {
        ...
        public void Setup()
        {
            ...
            Mock<Log> mockLog = new Mock<Log>(config);
            ...
            graph = new Graph(mockLog.Object, config);
        ...
        public void TryUnblockPeerAsync_TwoUnavailableOneSuccess()
        {
            ...
            mockConfig.Setup(x => x.MaxConnectionAttempts).Returns(4);
            //Act

            graph.TryUnblockPeerAsync("peer1", id);

            //Assert
            mockClient.Verify(x => x.SendAsync("UNBLOCK " + id, It.IsAny<bool>()), Times.Exactly(3));
        }
        ...
        public void TryUnblockPeerAsync_OneSuccess()
        {
            ...
            var success = Task.FromResult("SUCCESS");
            var unavailable = Task.FromResult("UNAVAILABLE");
            mockClient.Setup(x => x.SendAsync(It.IsAny<string>(), true))
                .Returns(success);
            config.MainNodes.Add("peer1", mockClient.Object);
            mockConfig.Setup(x => x.MaxConnectionAttempts).Returns(2);
        ...
        public void TryUnblockPeerAsync_AllUnavailable()
        {
            ...
            mockClient.Setup(x => x.SendAsync(It.IsAny<string>(), It.IsAny<bool>()))
                .Returns(unavailable);
            config.MainNodes.Add("peer1", mockClient.Object);
            mockConfig.Setup(x => x.MaxConnectionAttempts).Returns(5);

            //Act
            graph.TryUnblockPeerAsync("peer1", id);
            
            //Assert
            mockClient.Verify(x => x.SendAsync("UNBLOCK " + id, true), Times.Exactly(mockConfig.Object.MaxConnectionAttempts));
        }
