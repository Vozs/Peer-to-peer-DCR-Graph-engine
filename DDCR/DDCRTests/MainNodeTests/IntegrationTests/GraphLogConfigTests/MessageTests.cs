...
namespace DDCRTests.IntegrationTests.GraphLogConfigTests
{
    class MessageTests
    {
        ...
        public void MessagePeers_OnePeer_peer1SendResult()
        {
            ...
            mockClient.Setup(x => x.SendAsync(It.IsAny<string>(), true)).Returns(Task.FromResult("SUCCESS")).Verifiable();
            ...
            //Assert
            mockClient.Verify(x => x.SendAsync("COMMAND " + id.ToString(), true));
        ...
        public void MessagePeers_OnePeer_peer1And2SendResult()
        {
            ...
            var id = Guid.NewGuid();
            var mockClient = new Mock<IClient>();
            mockClient.Setup(x => x.SendAsync(It.IsAny<string>(), false)).Returns(Task.FromResult("SUCCESS")).Verifiable();
            config.MainNodes.Add("peer1", mockClient.Object);
            config.MainNodes.Add("peer2", mockClient.Object);
            //Act
            var result = graph.MessagePeers("COMMAND", id, prevTasks, false);

            //Assert
            mockClient.Verify(x => x.SendAsync("COMMAND " + id.ToString(), false));
            Assert.AreEqual("SUCCESS", result["peer1"].Result);
